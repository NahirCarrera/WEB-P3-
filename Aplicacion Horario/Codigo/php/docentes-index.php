            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Agregar Docentes Details</h2>
                        <a href="docentes-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="docentes-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                    ...
                    $orderBy = array('nombre', 'correo', 'estado');
                    $order = 'ID_docente';
                    ...
                    if(!empty($_GET['search'])) {
                        ...
                        $sql = "SELECT * FROM docentes
                            WHERE CONCAT_WS (nombre,correo,estado)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                        ...
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($row['nombre']) . "</td>";echo "<td>" . htmlspecialchars($row['correo']) . "</td>";echo "<td>" . htmlspecialchars($row['estado']) . "</td>";
                                        echo "<td>";
                                            echo "<a href='docentes-read.php?ID_docente=". $row['ID_docente'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='docentes-update.php?ID_docente=". $row['ID_docente'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='docentes-delete.php?ID_docente=". $row['ID_docente'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
