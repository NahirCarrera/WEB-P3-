            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">Agregar Periodos Details</h2>
                        <a href="periodos-create.php" class="btn btn-success float-right">Add New Record</a>
                        <a href="periodos-index.php" class="btn btn-info float-right mr-2">Reset View</a>
                    ...
                    $orderBy = array('codigo', 'nombre', 'fecha_inicio', 'fecha_fin');
                    $order = 'ID_periodo';
                    ...
                    if(!empty($_GET['search'])) {
                        ...
                        $sql = "SELECT * FROM periodos
                            WHERE CONCAT_WS (codigo,nombre,fecha_inicio,fecha_fin)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
										...
										echo "<th><a href=?search=$search&sort=&order=nombre&sort=$sort>Periodo</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . htmlspecialchars($row['codigo']) . "</td>";echo "<td>" . htmlspecialchars($row['nombre']) . "</td>";echo "<td>" . htmlspecialchars($row['fecha_inicio']) . "</td>";echo "<td>" . htmlspecialchars($row['fecha_fin']) . "</td>";
                                        echo "<td>";
                                            echo "<a href='periodos-read.php?ID_periodo=". $row['ID_periodo'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='periodos-update.php?ID_periodo=". $row['ID_periodo'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
                                            echo "<a href='periodos-delete.php?ID_periodo=". $row['ID_periodo'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
