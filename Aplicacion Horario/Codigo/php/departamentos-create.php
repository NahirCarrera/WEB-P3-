...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["codigo"]))){
        $codigo_err = "Please enter a código.";
    } else{
        $codigo = strtoupper(trim($_POST["codigo"]));
        if(!preg_match("/^[A-Za-z]{1,4}$/", $codigo)){
            $codigo_err = "El código debe contener solo letras y no puede exceder los 4 caracteres.";
        }
    }
